...
class NodeEmbedder(nn.Module):
    ...
    def embed_t(self, timesteps, mask):
        ...
        timestep_emb = timestep_emb.reshape(timesteps.shape[0], timesteps.shape[1],timestep_emb.shape[1])
        timestep_emb = timestep_emb[:, :, None, :].repeat(1, 1,mask.shape[1], 1)
        return timestep_emb * mask[:, None, :].unsqueeze(-1)

    def forward(self, timesteps, mask):
        # s: [b]

        b, num_res, device = mask.shape[0], mask.shape[1], mask.device
        num_timepoints = timesteps.shape[1]

        # [b, n_res, c_pos_emb]
        pos = torch.arange(num_res, dtype=torch.float32).to(device)[None, None].repeat(1, num_timepoints, 1)
        pos_emb = get_index_embedding(
            pos, self.c_pos_emb, max_len=2056
        )
        pos_emb = pos_emb.repeat([b, 1, 1, 1])
        pos_emb = pos_emb * mask[:,None,:].unsqueeze(-1)
